    ...
    <main className="min-h-screen">
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-6">
            ...
            <Link href="/trust" className="text-oxford text-sm font-medium">Trust</Link>
      ...
      <section className="pt-32 pb-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <h1 className="text-5xl font-serif font-bold text-oxford mb-6">Trust Center</h1>
      ...
      <section className="py-12 bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-4 gap-8 text-center">
            {stats.map((stat, i) => (
              <div key={i}>
                <div className="text-3xl font-bold text-oxford">{stat.value}</div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <h2 className="text-2xl font-serif font-bold text-oxford mb-8 text-center">Compliance Certifications</h2>
          <div className="grid md:grid-cols-2 gap-6">
            {certs.map((cert, i) => (
              <div key={i} className="bg-slate-50 p-6 rounded-lg">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="text-lg font-bold text-oxford">{cert.name}</div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="text-oxford">{cert.standards}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Encryption</span>
                    <span className="text-oxford">{cert.encryption}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          <div className="bg-white border border-slate-200 p-8 rounded-lg">
            <h2 className="text-2xl font-serif font-bold text-oxford mb-4">Data Residency & Processing</h2>
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                <h3 className="font-semibold text-oxford mb-2">Infrastructure</h3>
                <p className="text-slate-600 text-sm">All data stored in US-based SOC 2 certified data centers with redundant backups.</p>
              </div>
              <div>
                <h3 className="font-semibold text-oxford mb-2">Encryption Keys</h3>
                <p className="text-slate-600 text-sm">Keys rotated every 90 days. AWS KMS with HSM-backed key storage.</p>
              </div>
              <div>
                <h3 className="font-semibold text-oxford mb-2">Access Controls</h3>
                <p className="text-slate-600 text-sm">Role-based access with audit logging. No standing admin access.</p>
              </div>
              <div>
                <h3 className="font-semibold text-oxford mb-2">Incident Response</h3>
      ...
      <section className="py-20 bg-[#002147]">
        <div className="max-w-7xl mx-auto px-6 text-center">
          ...
          <div className="flex justify-center gap-4">
            <Link href="/security" className="bg-[#D4AF37] text-oxford font-semibold px-8 py-3 rounded-md hover:bg-[#b8962f] transition-colors">
              Security Details
            </Link>
